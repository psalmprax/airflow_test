...
--    )::BIGINT AS id
--	, COUNT(o.id) AS offers_sold_last_fourteen_days
...
--WHERE soi.created_at >= now() - INTERVAL '14 DAY'
--GROUP BY
--    CONCAT(
--        c.id,
--        m.id,
...
SELECT DISTINCT
	...
	, COUNT(o.id) AS offers_sold_last_fourteen_days
FROM {{ ref ("obj_wkfs.stock_out_info_cleaned") }} AS soi
    JOIN ( select id,latest_stock_id, CASE WHEN product_check_device_condition IS NULL THEN 5 ELSE product_check_device_condition end  AS condition,
    ...
    JOIN {{ ref("obj_wkfs.category_cleaned") }} AS c
        ON m.category_id = c.id
WHERE soi.created_at >= getdate() - INTERVAL '14 DAY'
GROUP BY
