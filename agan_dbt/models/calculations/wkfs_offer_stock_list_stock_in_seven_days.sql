...
--    )::BIGINT AS id
--	, COUNT(o.id) AS offers_stock_in_seven_days
...
--WHERE o.created_at >= now() - INTERVAL '7 DAY' AND o.latest_stock_id IS NOT NULL
--GROUP BY
--    CONCAT(
--        c.id,
--        m.id,
...
FROM {{ ref("obj_wkfs.offer_cleaned") }} AS o
    ...
    JOIN {{ ref("obj_wkfs.category_cleaned") }} AS c
        ON m.category_id = c.id
WHERE o.created_at >= getdate() - INTERVAL '7 DAY' AND o.latest_stock_id IS NOT NULL
GROUP BY
