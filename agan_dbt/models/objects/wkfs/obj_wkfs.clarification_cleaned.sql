with results as (
 select
	...
	convert(numeric, case donation_amount when '' then null else donation_amount end) as donation_amount,
	convert(numeric, case calculated when '' then null else calculated end) as calculated,
