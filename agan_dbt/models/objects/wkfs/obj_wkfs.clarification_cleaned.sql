with results as (
 select
	...
	COALESCE(convert(decimal(15,2), case donation_amount when '' then null else donation_amount end),0.00) as donation_amount,
	COALESCE(convert(decimal(15,2), case calculated when '' then null else calculated end),0.00) as calculated,
