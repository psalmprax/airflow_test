...
select
	...
	COALESCE(convert(decimal(15,2), case accepted_amount when '' then null else accepted_amount end),0.00) as accepted_amount,
	COALESCE(convert(decimal(15,2), case original_amount when '' then null else original_amount end),0.00) as original_amount,
	case
	...
	COALESCE(convert(decimal(15,2), case converted_amount when '' then null else converted_amount end),0.00) as converted_amount,
	COALESCE(convert(decimal(15,2), case commission_amount when '' then null else commission_amount end),0.00) as commission_amount,
